...
mod tests {
    ...
    async fn test_ping() {
        ...
        assert_eq!(response_text, "pong", "Expected 'pong' response, but received: '{}'", response_text);
    }
    ...
    async fn test_max_int_valid_types() {
        ...
        for int_type in valid_types {
            let args = MaxIntArgs { r#type: int_type.to_string() };
            let params = Parameters(args);
            
            let result = tokio::time::timeout(
                std::time::Duration::from_secs(3),
                server.max_int(params)
            ).await
            .expect(&format!("Max int {} timeout", int_type))
            .expect(&format!("Max int {} failed", int_type));
            
            // Verify response structure
            assert!(!result.content.is_empty(), "Max int {} should return content", int_type);
            ...
            assert!(!response_text.text.is_empty(), "Max int {} should return non-empty text", int_type);
    ...
    async fn test_max_int_invalid_type() {
        ...
        let args = MaxIntArgs { r#type: "invalid_type".to_string() };
        let params = Parameters(args);
        ...
        assert!(error.message.contains("Failed to get max int"), "Error should mention max int failure");
    }
    ...
    async fn test_min_int_valid_types() {
        ...
        for int_type in valid_types {
            ...
            let result = tokio::time::timeout(
                std::time::Duration::from_secs(3),
                server.min_int(params)
            ).await
            .expect(&format!("Min int {} timeout", int_type))
            .expect(&format!("Min int {} failed", int_type));
            
            // Verify response structure
            assert!(!result.content.is_empty(), "Min int {} should return content", int_type);
            ...
            assert!(!response_text.text.is_empty(), "Min int {} should return non-empty text", int_type);
    ...
    async fn test_max_uint_valid_types() {
        ...
        for uint_type in valid_types {
            let args = MaxUIntArgs { r#type: uint_type.to_string() };
            let params = Parameters(args);
            
            let result = tokio::time::timeout(
                std::time::Duration::from_secs(3),
                server.max_uint(params)
            ).await
            .expect(&format!("Max uint {} timeout", uint_type))
            .expect(&format!("Max uint {} failed", uint_type));
            
            // Verify response structure
            assert!(!result.content.is_empty(), "Max uint {} should return content", uint_type);
            ...
            assert!(!response_text.text.is_empty(), "Max uint {} should return non-empty text", uint_type);
    ...
    async fn test_address_zero() {
        ...
        // Verify response structure
        assert!(!result.content.is_empty(), "Address zero should return content");
        ...
        assert!(!response_text.text.is_empty(), "Address zero should return non-empty text");
        assert!(response_text.text.starts_with("0x"), "Address should start with 0x prefix");
        assert_eq!(response_text.text.len(), 42, "Ethereum address should be 42 characters");
    ...
    async fn test_hash_zero() {
        ...
        // Verify response structure
        assert!(!result.content.is_empty(), "Hash zero should return content");
        ...
        assert!(!response_text.text.is_empty(), "Hash zero should return non-empty text");
        assert!(response_text.text.starts_with("0x"), "Hash should start with 0x prefix");
        assert_eq!(response_text.text.len(), 66, "Keccak256 hash should be 66 characters");
            ...
            .map(|i| {
                ...
                tokio::spawn(async move {
                    match i % 4 {
                        ...
                        2 => {
                            let args = MaxIntArgs { r#type: "int32".to_string() };
                            server_ref.max_int(Parameters(args)).await
                        },
                        3 => {
                            let args = MaxUIntArgs { r#type: "uint64".to_string() };
    ...
    async fn test_tool_response_consistency() {
        ...
        for _ in 0..3 {
            ...
            assert_eq!(result1.content.len(), result2.content.len(), "Response lengths should match");
            
            let text1 = result1.content.first().unwrap().raw.as_text().unwrap().text.clone();
            let text2 = result2.content.first().unwrap().raw.as_text().unwrap().text.clone();
